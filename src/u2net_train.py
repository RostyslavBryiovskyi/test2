...
def train(config):
    ...
    tra_img_name_list = glob.glob(data_dir + tra_image_dir + os.sep + '*')[:200]
    ...
    ite_num4val = 0
    save_frq = 10 # save the model every 200 iterations
    
    mlflow.set_tracking_uri("http://localhost:5000")
    mlflow.set_experiment("BG removal")

    with mlflow.start_run(run_name=exp_name):
        mlflow.log_param("model", model_name)
        mlflow.log_param("batch size", batch_size_train)
        ...
        for epoch in range(0, epoch_num):
            ...
            for i, data in enumerate(salobj_dataloader):
                ...
                if ite_num % save_frq == 0:
                    ...
                    dvclive.log("train loss", running_loss / ite_num4val)
                    dvclive.log("target loss", running_tar_loss / ite_num4val)
                    dvclive.next_step()
                    mlflow.log_metric("train loss", running_loss / ite_num4val, step=ite_num)
                    mlflow.log_metric("target loss", running_tar_loss / ite_num4val, step=ite_num)
        ...
        dummy_input = torch.randn(1, 3, 320, 320, requires_grad=True, device="cuda")
